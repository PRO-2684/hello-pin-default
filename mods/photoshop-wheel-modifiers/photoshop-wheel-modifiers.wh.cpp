...
// @description     Makes Ctrl+wheel zoom, Shift+wheel scroll horizontally, and
// Alt+wheel scroll quickly
// @version         0.1
// @author          PRO-2684
...
becomes fast horizontal scroll, and **Alt + Ctrl + wheel** becomes fast zoom.

Keep **Preferences > Tools > Zoom With Scroll Wheel** disabled.

See the [full
README](https://github.com/PRO-2684/WindHawk-mods/tree/main/mods/photoshop-wheel-modifiers)
for details.
*/
// ==/WindhawkModReadme==
...
bool IsKeyForModifier(int virtualKey, ModifierMask modifier) {
  const bool isCtrl = virtualKey == VK_CONTROL || virtualKey == VK_LCONTROL ||
                      virtualKey == VK_RCONTROL;
  const bool isShift = virtualKey == VK_SHIFT || virtualKey == VK_LSHIFT ||
...
void LogMessage(const wchar_t *stage, const MSG *message) {
  ...
  const bool altPhysical = GetAsyncKeyState_Original &&
                           KeyDownFromState(GetAsyncKeyState_Original(VK_MENU));

  Wh_Log(L"%s WM_MOUSEWHEEL hwnd=0x%p class=\"%s\" wParam=0x%llX "
         L"lParam=0x%llX delta=%d msgKeys=[ctrl=%d shift=%d] "
         L"physical=[ctrl=%d shift=%d alt=%d]",
         stage, message->hwnd, className,
         static_cast<unsigned long long>(message->wParam),
         static_cast<unsigned long long>(message->lParam),
         static_cast<int>(wheelDelta), ctrlFromMessage, shiftFromMessage,
         ctrlPhysical, shiftPhysical, altPhysical);
}
...
SHORT WINAPI GetKeyState_Hook(int virtualKey) {
  ...
  if (g_modifierOverride != kModifierNone && IsModifierKey(virtualKey)) {
    Wh_Log(L"GetKeyState(%s): originalDown=%d returnedDown=%d override=%s",
           VirtualKeyName(virtualKey), KeyDownFromState(originalState),
           KeyDownFromState(state), ModifierName(g_modifierOverride));
  }
...
SHORT WINAPI GetAsyncKeyState_Hook(int virtualKey) {
  ...
  if (g_modifierOverride != kModifierNone && IsModifierKey(virtualKey)) {
    Wh_Log(L"GetAsyncKeyState(%s): originalDown=%d returnedDown=%d override=%s",
           VirtualKeyName(virtualKey), KeyDownFromState(originalState),
           KeyDownFromState(state), ModifierName(g_modifierOverride));
  }
...
BOOL WINAPI GetKeyboardState_Hook(PBYTE keyState) {
  ...
  if (g_modifierOverride != kModifierNone) {
    constexpr int modifierKeys[] = {
        ...
        VK_RSHIFT,  VK_MENU,     VK_LMENU,    VK_RMENU,
    };
  ...
  if (g_modifierOverride != kModifierNone) {
    Wh_Log(L"GetKeyboardState: original=[ctrl=%d shift=%d alt=%d] "
           L"returned=[ctrl=%d shift=%d alt=%d] override=%s",
           originalCtrl, originalShift, originalAlt,
...
ModifierMask GetPhysicalWheelModifier(WPARAM wParam) {
  ...
  if (GetKeyState_Original && KeyDownFromState(GetKeyState_Original(VK_MENU))) {
...
void ClearModifierOverride(const wchar_t *source) {
  if (g_modifierOverride != kModifierNone) {
    Wh_Log(L"%s: clearing wheel modifier override=%s", source,
           ModifierName(g_modifierOverride));
...
void RemapRetrievedWheelMessage(MSG *message, const wchar_t *source) {
  ...
  if (wcscmp(className, L"PSViewC") != 0) {
    Wh_Log(L"%s: WM_MOUSEWHEEL target class=\"%s\"; leaving unchanged", source,
           className);
    return;
  }
  ...
  Wh_Log(L"%s: WM_MOUSEWHEEL remap decision physical=%s remapped=%s", source,
         ModifierName(physicalModifier), ModifierName(remappedModifier));
  ...
  message->wParam = RewriteWheelKeyState(message->wParam, remappedModifier);
  g_modifierOverride = remappedModifier;

  Wh_Log(L"%s: WM_MOUSEWHEEL rewritten oldWParam=0x%llX newWParam=0x%llX "
         L"override=%s",
         source, static_cast<unsigned long long>(oldWParam),
         static_cast<unsigned long long>(message->wParam),
         ModifierName(g_modifierOverride));
}

BOOL WINAPI GetMessageA_Hook(LPMSG message, HWND hwnd, UINT messageFilterMin,
...
BOOL WINAPI GetMessageW_Hook(LPMSG message, HWND hwnd, UINT messageFilterMin,
                             UINT messageFilterMax) {
  // Re-entering GetMessage means Photoshop has finished processing the
  // previously retrieved message on this thread.
...
BOOL WINAPI PeekMessageA_Hook(LPMSG message, HWND hwnd, UINT messageFilterMin,
                              UINT messageFilterMax, UINT removeMsg) {
  BOOL result = PeekMessageA_Original(message, hwnd, messageFilterMin,
                                      messageFilterMax, removeMsg);
  if (!result) {
    return result;
  }
...
BOOL WINAPI PeekMessageW_Hook(LPMSG message, HWND hwnd, UINT messageFilterMin,
                              UINT messageFilterMax, UINT removeMsg) {
  BOOL result = PeekMessageW_Original(message, hwnd, messageFilterMin,
                                      messageFilterMax, removeMsg);
  if (!result) {
    return result;
  }
